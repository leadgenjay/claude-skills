...
// Each Item mode: return single object (not array)
return {
  json: {
    ...item.json,
    processed: true,
...
- **All Items mode**: Return array `[{json: {...}}, ...]`
- **Each Item mode**: Return single object `{json: {...}}`

---

## n8n v2 Breaking Change: .item Accessor Hang

**CRITICAL**: In n8n v2, using `$('NodeName').item.json` inside a Code node in **"Run Once for All Items" mode** causes the node to **hang forever** — no output, no error, execution stuck on "running."

```javascript
// ❌ BROKEN in n8n v2 — hangs silently in "All Items" mode
const raw = $('Mailbox Naming').item.json.output;

// ✅ FIXED — use .first() or $input.all()[0]
const raw = $('Mailbox Naming').first().json.output;
// or
const raw = $input.all()[0].json.output;
```

**Key details:**
- `.item` in **expression fields** (outside Code nodes) still works fine — n8n resolves those per-item automatically
- Only affects Code nodes in "Run Once for All Items" mode
- No error message is produced — the node simply never completes
- Replace `$('NodeName').item.json` with `$('NodeName').first().json` or `$input.all()[0].json`

**See**: [DATA_ACCESS.md](DATA_ACCESS.md) for full v2 compatibility details
