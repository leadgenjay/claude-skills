...
## n8n v2 Compatibility

### .item Accessor Silent Hang (CRITICAL)

In n8n v2, `$('NodeName').item.json` in **"Run Once for All Items" mode** Code nodes causes a **silent hang** — no error, no output, execution stuck forever.

```javascript
// ❌ BROKEN in n8n v2 — hangs silently
const data = $('Previous Node').item.json;

// ✅ FIXED — use .first() for single item
const data = $('Previous Node').first().json;

// ✅ FIXED — use .all()[0] for first item in batch
const data = $input.all()[0].json;
```

**Scope**: Only affects Code nodes — expression fields using `.item.json` still work fine.

### .first() vs .all() — Choose Correctly

`$input.first()` and `$('NodeName').first()` return only the **first item**. When the previous node outputs multiple items and every item must be processed, use `.all()` with a loop:

```javascript
// ❌ WRONG — only processes 1 of 600+ items
const result = $('Get Jobs').first().json;

// ✅ CORRECT — process all items
const allJobs = $('Get Jobs').all();
for (const job of allJobs) {
  // process job.json
}
```

**Rule of thumb**: Use `.first()` when expecting a single result (API response, config). Use `.all()` when processing a batch of records.
