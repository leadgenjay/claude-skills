...
### API Rejection: Think Tool Schema (Agent Node)

**What it means**: The AI provider API rejects the request at runtime due to a malformed Think tool schema auto-injected by the Agent node

**When it occurs**:
- Agent node at typeVersion ≤ 2.2
- Using Anthropic (Claude) or OpenAI (GPT-4o/5.1+) models

**Error messages**:
```
# OpenAI
400 Invalid schema for function 'Think': schema must be a JSON Schema of 'type: "object"'

# Anthropic
400 tools.0.custom.input_schema.type: Field required
```

**Fix**: Upgrade the Agent node `typeVersion` from 2.2 to **3**. No parameter changes needed.

**Note**: Not a validation-time error — surfaces only when the workflow executes and calls the AI API.
