...
For HTTP Request nodes calling external or unreliable services, always configure retry parameters:

```javascript
{
  // Standard retry settings for HTTP Request nodes
  "retryOnFail": true,
  "maxTries": 3,
...
**When to add retries:**
- External APIs with occasional timeouts or 5xx errors
- Webhook delivery endpoints
- Any service where transient failures are expected

**When NOT to retry:**
- 4xx client errors (bad request, unauthorized) — fix the request instead
- Idempotent-unsafe operations without deduplication
