...
### removeNode Uses `nodeName`

When removing a node via `n8n_update_partial_workflow`, the operation property is `nodeName` (not `name`):

```javascript
{
  type: "removeNode",
  nodeName: "Old Node"  // ✅ Correct property name
}
```

### queryReplacement Corruption

Complex SQL expressions in `queryReplacement` values can get corrupted with extra `}}` when updating workflows via MCP. For workflows with complex SQL:
- Edit complex SQL expressions directly in the n8n UI
- Or use inline SQL with n8n expressions instead of queryReplacement parameters
