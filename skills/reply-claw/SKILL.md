...
The agent runs autonomously: every 8 minutes it fetches new tracked replies from Email Bison (untracked inbox spam is filtered out before classification), sorts them into **11 categories** (`interested`, `booked`, `question`, `referral`, `follow_up_7d`, `follow_up_30d`, `not_interested`, `unsubscribe`, `do_not_contact`, `wrong_person`, `ooo`), and applies one of **8 custom Bison funnel tags** (`Interested`, `Hard No`, `Soft No`, `Auto Reply`, `Follow up 7d`, `Follow up 30d`, `Booked`, `Lead Magnet`) to the lead — exclusively, so each lead carries one funnel-state tag at a time. Drafts go to Telegram for review or auto-send booking confirmations on high-confidence interested replies. Every minute, it polls Telegram for slash commands (`/approve`, `/skip`, `/edit`, `/mute`, `/agent on|off|status`) and executes them against Bison and Turso. The `Lead Magnet` tag swaps in post-send when the drafter cited a lead-magnet KB ref.
