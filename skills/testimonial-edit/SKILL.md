...
```
[1] PROBE ─→ [2] TRANSCRIBE ─→ [3] AI SELECT ─→ [4] SILENCE ─→ [5] CROP
                                     ↓ (review gate)
[6] APPLY EDITS ─→ [7] ENHANCE ─→ [8] MANIFEST ─→ [9] RENDER ─→ [10] QA
```
...
| 6 | Apply Edits | FFmpeg trim+concat | edited.mp4 |
...
Auto-detected from ffprobe. AI face detection (face-api.js 68-point landmarks) extracts a frame at 2s and computes:
- **cropY**: Vertical position for square crop from vertical source
- **faceCenterX**: Horizontal face center (0-1) for X-axis centering
- **faceCenterY**: Vertical face center (0-1) for Y-axis centering

For vertical→square crops, if the face is >2% off-center horizontally, the crop window shrinks (up to 25% overcrop) and re-centers on the face, then upscales to 1080x1080. This handles off-center webcam framing common in Zoom testimonials. Override vertical position with `--crop-y`.
...
| Timing | Starts at 1s, duration 4s (configurable) |
...
| `lib/video/face-detection.ts` | face-api.js 68-point landmark detection → FacePosition (cropY, faceCenterX, faceCenterY) |
| `lib/video/testimonial-crop.ts` | FFmpeg crop/scale with horizontal face centering (25% max overcrop) + edit plan |
