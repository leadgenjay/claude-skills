...
### n8n v2 Breaking Change: _input.item Silent Hang

**CRITICAL**: In n8n v2, using `_input.item` inside a Code node in **"Run Once for All Items" mode** causes the node to **hang forever** — no output, no error, execution stuck on "running."

```python
# ❌ BROKEN in n8n v2 — hangs silently in "All Items" mode
raw = _input.item["json"]["output"]

# ✅ FIXED — use _input.all()[0] or _input.first()
raw = _input.all()[0]["json"]["output"]
# or
...
- Expression fields using `_input.item` outside Code nodes still work fine
