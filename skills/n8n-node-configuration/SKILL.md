...
## Critical Node Gotchas

### Think Tool Schema — Agent Node typeVersion ≤ 2.2

The Agent node's auto-injected Think tool is **broken** at typeVersion 2.2 and below. Both Anthropic and OpenAI APIs reject the malformed schema with errors like `Invalid schema for function 'Think'`.

**Fix**: Upgrade the Agent node `typeVersion` from 2.2 to **3**. No parameter changes needed — the config is compatible.

### Wait Node — Seconds Only

The Wait node only accepts **seconds**, never milliseconds. Use decimal seconds for sub-second waits:

```javascript
// ❌ WRONG
{ "amount": 300, "unit": "milliseconds" }

// ✅ CORRECT
{ "amount": 0.3, "unit": "seconds" }
```

**See**: [references/NODE_GOTCHAS.md](references/NODE_GOTCHAS.md) for Data Table, Google Sheets, and PostgreSQL configuration pitfalls

**See**: [references/N8N_V2_MIGRATION.md](references/N8N_V2_MIGRATION.md) for v1→v2 breaking changes checklist
