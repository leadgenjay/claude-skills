...
Delay execution - used for rate limiting and timing

#### Basic Wait

**Minimal** (seconds only!):
```javascript
{
  "amount": 5,
  "unit": "seconds"
...
**⚠️ CRITICAL GOTCHA**: Wait node ONLY accepts seconds, NOT milliseconds!

```javascript
// ❌ WRONG - milliseconds not supported!
{
  "amount": 300,
  "unit": "milliseconds"  // This does NOT work!
}

// ✅ CORRECT - use seconds (decimals allowed)
{
  "amount": 0.3,    // 300ms = 0.3 seconds
  "unit": "seconds"
}

// ✅ CORRECT - whole seconds
{
  "amount": 1,
  "unit": "seconds"
}
...
**For API rate limits** (e.g., 3 req/sec):
```javascript
{
  "amount": 0.3,    // ~3 requests per second
  "unit": "seconds"
}
```

**For conservative rate limits** (e.g., 1 req/sec):
```javascript
{
  "amount": 1,
  "unit": "seconds"
...
| Unit | Description |
|------|-------------|
| `seconds` | Wait N seconds (decimals allowed: 0.5, 1.5, etc.) |
| `minutes` | Wait N minutes |
| `hours` | Wait N hours |
...
| Utility | Wait | **Seconds only, NOT milliseconds!** |
