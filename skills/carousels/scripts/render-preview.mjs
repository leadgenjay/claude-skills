#!/usr/bin/env node
// render-preview.mjs — render the IG phone-mockup preview for a carousel.
//
// Fills the bundled template (assets/preview-templates/ig-carousel.html) with
// a JSON config and writes preview.html next to the slides.
//
// Usage:
//   node scripts/render-preview.mjs \
//     --out carousel-output/my-topic/preview.html \
//     --config '<inline-json>'          # OR --config @path/to/preview.json
//     [--template ig-carousel] [--open]
//
// Config shape (ig-carousel template):
//   { "handle": "clienthandle", "avatar": "avatar.png",
//     "slides": [{ "src": "slide-01.png" }, ...],
//     "caption": "...", "aspectRatio": "4:5",
//     "metrics": { "likes": "2,847", "comments": "61" },
//     "chrome": { "showStatusBar": true, "showBottomNav": true } }
// Slide/avatar paths are relative to the preview.html location.

import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { spawnSync } from "node:child_process";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const TEMPLATES_DIR = path.resolve(__dirname, "..", "assets", "preview-templates");

function parseArgs(argv) {
  const out = {};
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (!a.startsWith("--")) continue;
    const key = a.slice(2);
    const next = argv[i + 1];
    if (next === undefined || next.startsWith("--")) {
      out[key] = true;
    } else {
      out[key] = next;
      i++;
    }
  }
  return out;
}

function fail(msg, code = 1) {
  console.error(`render-preview: ${msg}`);
  process.exit(code);
}

const args = parseArgs(process.argv.slice(2));
const templateName = args.template || "ig-carousel";
if (!args.out) fail("missing --out <path>");
if (!args.config) fail("missing --config <json|@filepath>");

const templatePath = path.join(TEMPLATES_DIR, `${templateName}.html`);
if (!fs.existsSync(templatePath)) {
  const available = fs.existsSync(TEMPLATES_DIR)
    ? fs.readdirSync(TEMPLATES_DIR).filter((f) => f.endsWith(".html")).map((f) => f.replace(/\.html$/, ""))
    : [];
  fail(`template not found: ${templatePath}\navailable: ${available.join(", ") || "(none)"}`);
}

let configRaw;
if (typeof args.config === "string" && args.config.startsWith("@")) {
  const cfgPath = args.config.slice(1);
  if (!fs.existsSync(cfgPath)) fail(`config file not found: ${cfgPath}`);
  configRaw = fs.readFileSync(cfgPath, "utf-8");
} else {
  configRaw = args.config;
}

let config;
try {
  config = JSON.parse(configRaw);
} catch (e) {
  fail(`config is not valid JSON: ${e.message}`);
}
if (!config.template) config.template = templateName;

const template = fs.readFileSync(templatePath, "utf-8");
if (!template.includes("__PREVIEW_CONFIG__")) {
  fail(`template ${templateName}.html does not contain the __PREVIEW_CONFIG__ placeholder`);
}

const configJson = JSON.stringify(config).replace(/<\/script>/gi, "<\\/script>");
const html = template.replace("__PREVIEW_CONFIG__", configJson);

const outPath = path.resolve(args.out);
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, html);

const jsonPath = path.join(path.dirname(outPath), "preview.json");
fs.writeFileSync(jsonPath, JSON.stringify(config, null, 2));

console.log(JSON.stringify({ status: "ok", template: templateName, preview: outPath, config: jsonPath }, null, 2));

if (args.open) {
  const opener =
    process.platform === "darwin" ? "open" : process.platform === "win32" ? "start" : "xdg-open";
  const r = spawnSync(opener, [outPath], { stdio: "inherit", shell: process.platform === "win32" });
  if (r.status !== 0) console.error("render-preview: could not auto-open — open the file manually in a browser");
}
